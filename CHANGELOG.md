...
- `display(...)` no longer rejects arguments named like `#[oopsie(...)]`
  keywords, such as `path = path.display()`. In the short form
  `#[oopsie("…", args…)]`, `format!` gets the arguments it uses and the
  remaining keywords apply to the item.
...
[0.1.1]: https://github.com/gaetschwartz/oopsie/releases/tag/v0.1.1
