...
use oopsie::{IntoError, Oopsie, ResultExt};
use std::error::Error;
use std::io;
...
enum ProvideError {
    ...
    #[oopsie("help error")]
    #[oopsie(provide(::oopsie::HelpText => ::oopsie::HelpText("try rebooting")))]
    WithHelp { msg: String },
...
enum InnerError {
    #[oopsie("inner: {detail}")]
    #[oopsie(provide(ref, oopsie::Backtrace => bt.as_ref()))]
    #[oopsie(provide(::oopsie::HelpText => ::oopsie::HelpText("fix the inner thing")))]
...
/// Test that the `#[oopsie]` attribute macro (batteries-included) also
/// properly propagates backtrace through nested errors.
#[oopsie::oopsie]
#[derive(Debug)]
...
fn nested_attr_macro_backtrace_propagated() {
    ...
    // The #[oopsie] attr macro injects backtrace with provide(ref, Backtrace => ...)
