...
/// Re-exec this test binary, running only `panic_child` with the trigger set,
/// and return its (ANSI-stripped) stderr.
fn run_panicking_child(extra_env: &[(&str, &str)]) -> String {
...
fn renders_header_message_and_hint_without_backtrace() {
    let stderr = run_panicking_child(&[("NO_COLOR", "1"), ("RUST_BACKTRACE", "0")]);
...
fn renders_backtrace_with_user_frame_on_top_and_no_panic_plumbing() {
    let stderr = run_panicking_child(&[("NO_COLOR", "1"), ("RUST_BACKTRACE", "1")]);
